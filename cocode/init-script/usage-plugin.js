...
var _getModulesListForTargetVersion = _interopRequireDefault(
...
function _default(
	...
	const addAndRemovePolyfillImports = {
		ImportDeclaration(path) {
			if (
				(0, _utils.isPolyfillSource)((0, _utils.getImportSource)(path))
			) {
		...
		Program: {
			enter(path) {
				path.get('body').forEach(bodyPath => {
					if (
						(0, _utils.isPolyfillSource)(
							(0, _utils.getRequireSource)(bodyPath)
						)
					) {
						console.warn(NO_DIRECT_POLYFILL_IMPORT);
						bodyPath.remove();
	...
	return {
		...
		pre() {
			...
			this.addBuiltInDependencies = function(builtIn) {
				if ((0, _utils.has)(_builtInDefinitions.BuiltIns, builtIn)) {
					const BuiltInDependencies =
						_builtInDefinitions.BuiltIns[builtIn];
			...
			this.addPropertyDependencies = function(source = {}, key) {
				...
				if (
					!(0, _utils.has)(
						_builtInDefinitions.InstanceProperties,
						key
					)
				)
					return;
				...
				if (instanceType) {
					InstancePropertyDependencies = InstancePropertyDependencies.filter(
						m =>
							...
							_builtInDefinitions.CommonInstanceDependencies.has(
								m
							)
					);
				}
